...
# note: If on windows machine make sure to use method = "wget" check out ?download.file() for different options. 
...
## Cleaning the data
We shall start with making the data readable from the column names and other variables. 
...
dat <- dat[, (names(dat) %in% c("YEAR","DISTRICT","NUMBER.OF.SCHOOLS.UNEB.CENTRE", "OTAL.CANDIDATES", 
...
```{r districtCount}
districts_PLE <- table(data$DISTRICT)
districts_PLE
```
...
We do notice that each District got the REAL name then SPACE then an additional name like M/C or MUN e.g MBARARA M/C,IGANGA MUN. One other strategy we could use is removing anything after the first keyword.  
...
# Using the base Plot. 
plot(dat$FEMALE.CANDIDATES,dat$MALE.CANDIDATES)
...
# Using the base Hist. 
hist(dat$FEMALE.CANDIDATES, main = "Distribution of FEMALE CANDIDATES", breaks = 30, xlab = "count (n)", col = "red")
...
# Make sure the Year is a factor. using as.factor()
dat$YEAR <- as.factor(dat$YEAR)
abim_dat <- dat[which(dat$DISTRICT=='ABIM'),]
# dim(newdata)
# Try "o" "p" "l" "b"
plot(abim_dat$YEAR, abim_dat$TOTAL.DIV.1, type ="l",  col = "red" )
abline(lm (TOTAL.DIV.1 ~ YEAR, data = abim_dat), lty = "dashed")
...
# bg = "blue"
# cex = 2
plot(dat$FEMALE.CANDIDATES, dat$MALE.CANDIDATES, col = dat$YEAR)
...
#notch = T
#las = 1
boxplot(dat$TOTAL.DIV.1 ~ dat$YEAR)
...
hist(dat$MALE.CANDIDATES, main = "Distribution of MALE CANDIDATES", breaks = 30, xlab = "count (n)", col = "red")
